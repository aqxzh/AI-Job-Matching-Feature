...
const faqQuestions = [
  {
    ...
    answer: 'Проверить используя другой браузер, очистить кеш. Если не получается зарегестрироваться то удостовериться в дачи согласие посредством смс сообщения. В момент когда к вам приходит сообщение с 1414 согласие даёте, отправляете смс с сообщением 511'
  },
  {
    id: 2,
    category: 'Восстановление',
    question: 'Забыл пароль',
    answer: 'Кандидат может воспользоваться функцией "Забыли пароль", также вы можете обратиться в службу поддержки указанную на портале'
  },
  {
    id: 3,
  ...
  },
  {
    ...
    answer: 'Обратитесь в службу поддержки которая указана на портале в разделе помощь или на главной странице. Посродством ватсап или же jivo чат'
  },
  {
    id: 10,
    category: 'ОЛК',
  ...
  },
  {
    ...
    answer: '"Онлайн сдать ОЛК тест можно только при отклике на вакансию. На портале е-қызмет нажимаете ""Откликнуться"", далее система предложит Вам записаться на ОЛК тест, выбираете дату, время, язык и записываетесь. Запись будет в личном кабинете в разделе ""Мои заявки"""'
  },
  {
    id: 12,
    category: 'Результаты',
    question: 'Когда выйдет результат? Как узнать результать? Вчера сдавал(а) тест',
    answer: 'Вам придет уведомление, также результат вы сможете увидеть после проверки проктора в разделе мои заявки'
  },
  {
    id: 13,
    category: 'Проверка',
  ...
  },
  {
    ...
    answer: '3) Кандидат со своей стороны обеспечивает наличие компьютера/ноутбука с браузером Chrome (версия не менее 95.х.х.), Opera, Microsoft Edge стабильного интернета со скоростью не менее 5 мб/сек, функционирующей веб-камеры и микрофона, хорошее освещение, отсутствие посторонних звуков/шумов и людей. 4) Обращаем ваше внимание, что рекомендуется использовать персональное интернет- соединение (например, домашний Wi-Fi). Согласно Постановлению Правительства Республики Казахстан от 20 декабря 2016 года № 832 5) Об утверждении единых требований в области информационнокоммуникационных технологий и обеспечения информационной безопасности, корпоративные сети могут быть ограничены единым шлюзом доступа в интернет, что может вызвать перебои в соединении либо невозможность подключения к системе тестирования. Также можете обратиться в службу поддержки'
  },
  {
    id: 15,
    category: 'Тестирование',
    question: 'Ошибки во время теста вышли. Я не прошел?',
    answer: 'Система прокторинга автоматически реагирует на каждое движение и фиксирует нарушения, далее проктор проверяет видео и в течении 24 часов выносит решение. Просим ожидать решение проктора'
  },
  {
    id: 16,
    category: 'Запись',
    question: 'Как записать ся на второй этап?',
    answer: 'На второй этап нужно записываться за 24 часа до начала второго этапа в личном кабинете в разделе "Мои заявки" будет кнопка "записаться на следующий этап"'
  },
  {
    id: 17,
    category: 'НПА',
    question: 'Как записаться на НПА?',
    answer: 'На портале е-қызмет в разделе Тестирование на правой стороне есть кнопка "Запись на тестирование". Нажимаете на нее, далее выбираете дату, время, язык и подтверждаете запись. Запись будет в личном кабинете в разделе "Мои заявки"'
  },
  {
    id: 18,
    category: 'Запись',
    question: 'Не могу выбрать дату и записаться',
    answer: 'Убедитесь что в календаре указан правильный месяц'
  },
  {
    id: 19,
    category: 'Тестирование',
    question: 'Как сдать тестирование? Где сдать тестирование?',
    answer: 'В личном кабинете, в разделе Мои заявки будет кнопка "Начать"'
  },
  {
    id: 20,
    category: 'Тестирование',
    question: 'Статус тестирования за оценку личных качеств стоит аннулирован',
    answer: ' В случае несоответствия кандидата квалификационным требованиям и (или) непредоставления либо отсутствия документов, предусмотренных настоящим пунктом и представления неполного пакета документов, либо недостоверных сведений, либо несоответствия документов предъявляемым к ним требованиям, кандидату отказывается в допуске к следующему этапу общего конкурса и системой направляется уведомление об отказе в допуске к прохождению этапов общего конкурса.'
  },
  {
    id: 21,
    category: 'ОЛК',
    question: 'Нажала откликнуться, но отклика нет',
    answer: 'Вы записались на ОЛК? Пожалуйста, убедитесь, что Ваша запись отображается в личном кабинете в разделе Мои заявки. '
  },
  {
    id: 22,
    category: 'Тестирование',
    question: 'Отправили на доработку',
    answer: 'Если ваше резюме отправили на доработку, вам нужно внести все изменения требуемые кадровой службой, в раздел резюме до срока который указан в уведомлении'
  },
  {
    id: 23,
    category: 'Запись',
    question: 'Сколько кандидатов проходт на практическую оценку?',
    answer: '"К этапу собеседование посредством практической оценки (далее –практическая оценка) допускаются два кандидата, набравшие наивысшие баллы по результатам предыдущих этапов общего конкурса. В случае, если пороговое значение по результатам предыдущих этапов общего конкурса набирает один кандидат, допускается участие одного кандидата. При равенстве баллов кандидатов, набравших наивысшие баллы по результатам предыдущих этапов общего конкурса, допускается превышение установленного количества кандидатов, направляемых на практическую оценку."'
  },
  {
    id: 24,
    category: 'НПА',
    question: 'Когда будет оценка за практический этап?',
    answer: '"В день завершения выполнения всех трех заданий результаты направляются на проверку проктору. Проверка проктором проводится в течение одного следующего рабочего дня. В случае отсутствия нарушений требований Правил кандидатом, в течение одного рабочего дня после объявления результатов проверки проктора непосредственный руководитель структурного подразделения государственного органа, имеющего вакантную должность, на которую объявлен общий конкурс, вносит в систему итоги этапа практической оценки по форме согласно приложению 2 к настоящим Правилам."'
  },
  {
    id: 25,
    category: 'Запись',
    question: 'Не получается откилкнутся на вакансию, нет сертификата НПА',
    answer: 'Наличие у кандидата сертификата тестирования на знание государственного языка и законодательства Республики Казахстан определяется системой. Если у вас нет действующего сертификата НПА. Вам нужно сдать тестирование на знание гос языка и законов, Онлайн на портале екызмет или же офлайн в зале тестирования, предварительно записавшись через егов'
  },
  {
    id: 26,
    category: 'Тестирование',
    question: 'Проблемы с интернетом, вылетело из за камеры или микрофона',
    answer: '"Кандидат со своей стороны обеспечивает наличие компьютера/ноутбука с браузером Chrome (версия не менее 95.х.х.),Opera, Microsoft Edge стабильного интернета со скоростью не менее 5 мб/сек, функционирующей веб-камеры и микрофона, хорошее освещение, отсутствие посторонних звуков/шумов и людей.'
  },
];
